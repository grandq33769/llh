...
def predict(name,box_list,image):
    ...
    if(now_index != 0):
        ...
        if(now_index%2 == 0):
            ...
            for box in blist:
                return_list.append(getCalResult(model,threshold,image,box,input_size))
...
def getCalResult(model,threshold,image,box,input_size):
    ...
    if(result_list != []):
        result = calibrationTransform(box,tuple(sum(i)/len(result_list) for i in zip(*result_list)))
    else:
        result = box
...
def needMerge(first,second,threshold):
    if (is_overlap(first,second)):
        income_x,income_y,origin_x,origin_y = setOfIntersection(first,second)
        intersection = len(income_x - origin_x)  * len(income_y - origin_y)
        total = len(income_x) * len(income_y) + len(origin_x) * len(origin_y) - intersection
