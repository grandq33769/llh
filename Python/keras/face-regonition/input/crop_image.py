...
def crop_negative(face_list, image, image_size):
    ...
    for x_start in range(0,width,image_size):
        for y_start in range(0,height,image_size):
            rbox = (x_start,y_start,x_start+image_size,y_start+image_size)
            if all([is_overlap(rbox,loc) for loc in location_list]):
...
for filename in FILESET:
    with Image.open(URLBASE + '/' + filename + '.jpg', 'r') as image:
        face_list = LOCATION_DICT[filename]
        croped_list = crop_negative(face_list, image)
        file_name = filename.replace('/','_')
